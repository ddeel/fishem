...
"""
FishDoctor API Definitions.

Defines REST API behaviors for FishDoctor.
...
from fish_data import fishem_config     # fishem config parameters
...
#
# None; FishDoctor has unrestricted access to all fish objects
...
# mockups or fish files are loaded into fishem.
#
# None; FishDoctor has unrestricted access to all fish objects


class FishDoctor(Resource):
    """Defines API behaviors for the FishDoctor, which can do
    Create, Read, Update, and Delete (CRUD) operations on all
    fish objects.
    ...
    def put(
        ...
        if inst_key.endswith('/'):
            ...
            inst_key = inst_key.replace('//', '')
        # Ensure object is in the fish object dictionary
        # TODO: Determine if Fishdoctor needs this check
        if inst_key not in fish:
            return 'Object not found', HTTP.NOT_FOUND
        ...
        # Ensure @odata.id always matches the fish object key
        new_key = json_input['@odata.id']
        if new_key.endswith('/'):
            new_key += '/'
    ...
    def patch(
        ...
        # Return a copy of the updated fish object
        return fish[inst_key], HTTP.OK
        # End of patch()
    ...
    def delete(
        ...
        # Ensure collection is in the fish object dictionary
        if coll_key not in fish:
            return 'Collection not found', HTTP.NOT_FOUND
        # Ensure object is in the Collection's Members
...
class FishDoctorActions(Resource):
    """Defines API behaviors for FishDoctor Actions.
    ...
    def post(
        ...
        if action_name == '':
            return 'Unknown Action', HTTP.BAD_REQUEST
        elif action_name == 'FishDoctor.Remove':
            # Removes a fish object without any other adjustments
            # (does not assume the object is a collection member);
            # Get a copy of the object and remove it from the fish
            deleted_object = fish[inst_key]
            del fish[inst_key]
            # Return a copy of the deleted object
            return deleted_object, HTTP.OK
...
def activate(rest_api):
    ...
    # Register FishDoctor URIs with Flask only if it is enabled;
    # Flask will then accept or reject attempts to use FiskDoctor
    if not fishem_config['fishdoctorEnabled']:
        return

    # Register the URIs this API module responds to:
    rest_api.add_resource(
        FishDoctor,
        '/fishdoctor/<path:UriPath>',
        '/fishdoctor/<path:UriPath>/'
        )
    ...
    rest_api.add_resource(
        FishDoctorActions,
        '/fishdoctor/<path:UriPath>/Actions/<string:UriAction>',
        '/fishdoctor/<path:UriPath>/Actions/<string:UriAction>/'
        )
